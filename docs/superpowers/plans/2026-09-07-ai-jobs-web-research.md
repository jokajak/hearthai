...
The synchronous response is the terminal `WebResearchResult`. If the Open WebUI
integration requires a longer-lived callback mechanism, that adapter owns continuation;
the public research payload does not acquire job-management fields.
...
### Task 1: Create executable contracts and fixtures
...
- [ ] Publish only `/healthz`, `/readyz`, and `POST /v1/tools/web-research` in OpenAPI;
  keep worker routes in a separate non-model-facing schema.
...
- [ ] Store request/result data only as long as continuation and debugging policy needs;
...
- [ ] Verify the actual Open WebUI tool-registration mechanism during implementation;
  record its versioned request/timeout/continuation assumptions in the runbook rather
  than guessing at them in the core contract.
...
- [ ] Render a fixed image and entry point, run ID plus one-time token reference,
  resources, active deadline, retry count, TTL, labels, and restrictive pod security.
...
- [ ] Test that questions, user/conversation IDs, run IDs, URLs, citations, page content,
  tokens, and credentials never become metric labels or ordinary log fields.
- [ ] Trace using an opaque correlation value that operators can connect across control
  plane and worker logs without exposing it to the model-facing result.
...
**Acceptance:** A versioned release contains the two images and chart; rendered manifests
show least privilege and a fixed web-research profile.
...
- Modify `docs/ARCHITECTURE.md` only after implementation status changes
- Modify `docs/ROADMAP.md` only after milestone acceptance passes
...
- [ ] Document required `home-ops` resources: dedicated namespace, chart pin, persistent
  ...
  plane/provider egress, Open WebUI registration, and metrics scraping.
