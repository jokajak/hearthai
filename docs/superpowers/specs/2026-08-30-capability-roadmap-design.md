...
> **Superseded for 0.3:** The Open Terminal approach below is retained as historical design context. It was briefly replaced by an [`ai-jobs` web-research plan](../plans/2026-09-07-ai-jobs-web-research.md), which was itself withdrawn on 2026-09-07 for specifying the wrong capability. The current implementation direction is the [`hearthfetch` plan](../plans/2026-09-07-hearthfetch-quarantined-web-tool.md): a narrow OpenAPI tool server whose tools fetch a page, read it with a quarantined model holding no tools or context, and return deterministically-scrubbed distillations — the dual-LLM pattern. The `search_web`/`fetch_url` facade shape below survives; what changed is that the facade now contains the quarantined model, and search results are opaque handles rather than URLs. Note that 0.3 is no longer a model-facing `search_web`/`fetch_url` tool pair at all — it sits inside OpenWebUI's retrieval pipeline.
