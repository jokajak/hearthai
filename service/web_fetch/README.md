...
```
request ─▶ webfetch-fetch ──sealed handoff──▶ webfetch-inspect ─▶ envelope
...
* [`contracts/`](contracts) — the wire types both stages and the control plane
  ...
  the sealed handoff between the stages, the supported media set and the limits.
...
* [`inspector/`](inspector) — decode, scan, convert, scan again. Its states are
  the gate: each one consumes the last, and the only way to obtain releasable
  content is to have gone through all of them.
