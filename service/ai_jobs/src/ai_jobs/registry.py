...
class ToolDefinition(Protocol):
    ...
    def storage_view(self, request: object) -> Mapping[str, object]:
        """What the durable run record may keep about this request.

        Tools that handle caller-supplied locations return digests here rather
        than the values themselves, so the run store never holds a full URL.
        """
        ...
