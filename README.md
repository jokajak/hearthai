...
**Status: the shared-memory skill and service are built; the next work is making HearthAI a useful daily work surface.**
...
- **GitHub first:** HearthAI works on any repository Josh explicitly authorizes. Every model-callable capability runs in a fixed, ephemeral sandboxed pod; GitHub PR work is the first profile used to validate that substrate. Its worker gets an ephemeral repository workspace but no GitHub credential; after it terminates, `ai-jobs` starts a separate publisher pod that uses a repository-scoped installation token to create a branch, commit, and PR as `hearthai[bot]`. It never merges. Every run has an immutable audit record.
...
| [`deploy/charts/hearthmem/`](deploy/charts/hearthmem/) | A Helm chart. Agents run per person and reach the service over the network, so it has to live somewhere they can all get to. One replica, a persistent volume, non-root with a read-only root filesystem. |
| [`.github/workflows/`](.github/workflows/) | Tests on three Pythons, the image built and smoke-tested on every PR, chart rendered and schema-validated. Tagging `v*` publishes a multi-arch image and the chart to GHCR. |
